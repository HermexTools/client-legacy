	...
	public SystemTrayMenu() {
		...
		if (isSupported()) {
			try {
				...
				trayIcon = new TrayIcon(new ImageIcon(SystemTrayMenu.class.getResource("/icon.png")).getImage(), "KSUploader");
				...
				new Thread() {
					...
					public void run() {
						...
						if (Main.config.getFtpEnabled()) {
							...
							if (selFile.getSelectedFiles().length > 1 || selFile.getSelectedFiles()[0].isDirectory()) {
								Main.dialog.setButtonClickable(false);
								socketUploader.setFilePath(Zipper.toZip("socket", selFile.getSelectedFiles(), selFile.getSelectedFiles()[0].getParentFile().getPath()));
								res = socketUploader.send("{0}.zip");
							} else {
								socketUploader.setFilePath(selFile.getSelectedFile().getPath());
								res = socketUploader.send(selFile.getSelectedFile().getName());
							}
